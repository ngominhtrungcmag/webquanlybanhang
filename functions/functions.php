	...
	if (isset($_GET['add_cart'])) {
		...
		if (mysqli_num_rows($run_check_pro) > 0) {
			...
			if ($cart_SoLuong <= $soLuongKho) {
				...
				echo "
					<script>
					window.alert('Thanh cong! $cart_SoLuong');
	...
	while ($row_carts = mysqli_fetch_array($run_carts)) {
		...
		echo "
					...
					<div class='produc-shopping-left'>
						<img src='/quanlybanhang/admin/product_images/$cart_Hinh' alt='$cart_Ten'>
					</div>
...
function update_cart_del($product_id)
{
	...
	if ($fetch_pro) {
		...
		<script>window.open(quanlybanhang/cart.php,'_self')</script>
		";
	}
}
				...
				<div class='col-md-4 mt-3'>
    		    	<div class='card profile-card-5'>
    		        	<div class='card-img-block'>
    		            	<img class='card-img-top' src='/quanlybanhang/admin/product_images/$pro_image' alt='$pro_title'>
    		        	</div>
                    	<div class='card-body pt-0'>
                    		<h5 class='card-title'>$pro_title</h5>
							<span class='price-new'>Giá $pro_price</span> <del class='price-old'></del>
