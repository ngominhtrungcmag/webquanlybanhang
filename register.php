        ...
        if (isset($_POST['register'])) {
            ...
            if (!empty($_POST['usernamekh']) && !empty($_POST['passwordkh']) && !empty($_POST['confirm_password']) && !empty($_POST['name'])) {
                ...
                if ($usernamekh_count > 0) {
                    echo "<script>alert('Sorry, your email $usernamekh address already exist in our database !')</script>";
                    ...
                    if ($run_insertkh && $run_insertdn) {
                        $sel_user = mysqli_query($con, "select * from khachhangdangnhap where email='$usernamekh' ");
